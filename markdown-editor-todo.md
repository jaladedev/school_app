...
- Still open, for a future pass rather than blocking this entry: full color-contrast ratio audit (spot-checked marigold-on-white and ink-soft text, not measured systematically), and dyslexia font is a CSS fallback stack rather than a bundled purpose-built font since there's no font-CDN access from this build.
