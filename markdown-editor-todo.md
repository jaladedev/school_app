...
## 11. Better Preview — DONE

Was: fixed 2-column Markdown/Preview grid (no toggle modes). Now: `NoteWorkspace.tsx` owns a top mode pill — Edit / Preview / Student view / Present — sitting above the (single, WYSIWYG) `NoteEditor`.

- ✅ Preview (read-only render) — `forcePreview` prop threaded through to `NoteEditor`, forces `editor.setEditable(false)` and hides the toolbar/mobile-tab chrome; renders the same live TipTap doc as Edit mode (including any unsaved changes), not a separate re-parse
- ✅ Student View — reuses `TopicContent` directly, the same component that renders the note on the actual student-facing topic page, so this mode is a true preview of what a student sees, not an approximation
- ✅ Presentation View — `NoteSlideView` + `BellTimer` (the countdown a teacher projects while presenting lives here specifically, not the teacher dashboard, since a student in the room never sees the dashboard)
- ❌ Split View — deliberately **not** built; see #12's reasoning, which applies here too. Post-TipTap, edit already _is_ the render (no separate markdown pane to split against), so there's nothing for a side-by-side mode to show that Edit mode doesn't already show. Only reopen if a genuine second representation (e.g. raw markdown source) gets added later.

Student view/Present are disabled (`title` explains why) until the note has been saved at least once (`noteId` present) — Preview needs no such gate since it renders the in-progress draft, not the saved row. Present specifically shows the **last-saved** version, not live edits — switching tabs doesn't lift unsaved editor state up, matching how the rest of the page already treats "current content" (e.g. resource lists only refresh after a save's `revalidatePath`).
...
**Extended to Quiz Builder — DONE.** `QuestionText.tsx` already rendered `$...$`/`$$...$$` via the same `remark-math`+`rehype-katex` pipeline as notes, but `QuizBuilder.tsx`'s question-text field was a plain `<textarea>` with no way to compose an equation beyond typing raw LaTeX by hand. Added `components/MathInsertButton.tsx`: "∑ Inline math"/"∑ Block math" toolbar buttons per question, opening a popover with the same Visual (MathLive)/LaTeX toggle and quick-symbol row as the note editor's math node — but built standalone rather than reusing that node directly, since quiz `question_text` is a plain markdown string, not a TipTap doc, so there's no ProseMirror NodeView to hook into. On Insert, splices `$latex$` (inline) or a blank-line-wrapped `$$latex$$` (block) into the textarea at the caret via a per-question `Map<number, HTMLTextAreaElement>` ref, then refocuses.

- **On-screen keyboard left orphaned after closing the popover.** Same root cause hit in both `MathInsertButton.tsx` and (turned out) the original note editor's math node itself: MathLive's virtual keyboard renders as a global overlay appended to `document.body`, entirely outside the popover/popup's own DOM subtree. Two separate symptoms from this: (1) the outside-click handler didn't know to treat clicks on the keyboard as "inside," so tapping any key closed (unmounted) the popover mid-interaction, orphaning the keyboard panel — no field left wired to it, which is why backspace/keys stopped doing anything; (2) even once that's fixed, `math-virtual-keyboard-policy="manual"` means mathlive never auto-hides the keyboard on its own, so every _legitimate_ close (commit, cancel, a real outside click, switching Visual→LaTeX) needs to call `mathVirtualKeyboard.hide()` explicitly or the panel is left sitting on screen indefinitely. Fixed in both files: added the `isMathliveUi` outside-click exception (`math-nodes.tsx` already had this half; `MathInsertButton.tsx` didn't), plus a `hideVirtualKeyboard()` helper (reads `window.mathVirtualKeyboard` — a runtime global mathlive attaches to `window`, not a module export) wired into every close path.
- Question-text textarea placeholder simplified from `"Question text — use $...$ for inline math, e.g. $x^2 + 3x - 4 = 0$"` to just `"Question text"` now that the toolbar buttons cover that.

## 19a. Quiz Scheduling Time Picker — DONE

`QuizBuilder.tsx`'s "Opens at"/"Closes at" fields went through three iterations while sorting out a real cross-browser time picker:

1. Started as native `type="datetime-local"` inputs. Several browsers (Firefox included) only expose a date-picker popup for these — the time segment has to be edited directly inside the input itself, which reads as "time isn't editable" if you only ever try the popup.
2. Split into a native `type="date"` + `type="time"` pair (`splitDateTime`/`joinDateTime` helpers combine them back into the same `"YYYY-MM-DDTHH:mm"` string the rest of the code — validation, `new Date(...).toISOString()` on submit — already expected, so no downstream changes needed). First pass gated the time input on `disabled={!opensAt}` (must pick a date first); removed after the same "can't edit time" report came back — a disabled field with no visible explanation reads as broken. `joinDateTime` now defaults whichever side is still blank (today's date / midnight) so the fields work in either order.
3. Replaced native `type="time"` with `components/TimeSelect.tsx` — two dropdowns (hour, minute in 5-min steps) built as a small trigger button + capped-height (`max-h-40`) `overflow-y-auto` list, rather than either a native `<select>` (whose popup height/scroll behavior is entirely browser-controlled, can't be styled short) or the native time input (inconsistent editing UX across browsers). Same `"HH:mm"` value shape throughout, so `QuizBuilder.tsx` only swaps the component, not the date-joining logic.

- `onInsert`/`onChange` renamed to `onInsertAction`/`onChangeAction` on `MathInsertButton`/`TimeSelect`'s public props (Next.js's client-boundary serializable-props convention: a function prop on a `"use client"` component needs to end in `Action` unless it's already named `action`).
