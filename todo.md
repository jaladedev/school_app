...
- [x] **Hostel visitor log** — the table, RLS, audit trigger (`hostel_visitor_log_migration.sql`) and the UI (`RoomOccupants.tsx` visitor badge/check-in form, per the Hostel module section above) were already built in an earlier session. Migration confirmed applied — `hostel_visitor_logs` table, RLS, and audit trigger all present live.
- [x] **Driver app: mark pickup/drop** — `transport_pickup_logs` table + RLS (`transport_pickup_log_migration.sql`), `markStudentPickup()`, `StudentPickupChecklist.tsx` on `/dashboard/driver` all already built. Migration confirmed applied — `transport_pickup_logs` table and RLS present live.
