...
All previously-pending migrations are now confirmed applied via a live schema dump (hostel tables, `transport_locations`, `is_driver_of_route()`, `vehicles.driver_profile_id`, `profiles_select_staff`, and the locked-down `record_invoice_payment`/`log_receipt_print` grants are all present in the live DB):
...
- [~] **Hostel visitor log** — the table, RLS, audit trigger (`hostel_visitor_log_migration.sql`) and the UI (`RoomOccupants.tsx` visitor badge/check-in form, per the Hostel module section above) were already built in an earlier session. Confirmed still true, and confirmed the migration is genuinely **not yet applied** — `hostel_visitor_logs` doesn't appear anywhere in the current baseline schema dump.
- [~] **Driver app: mark pickup/drop** — same situation as visitor log: `transport_pickup_logs` table + RLS (`transport_pickup_log_migration.sql`), `markStudentPickup()`, `StudentPickupChecklist.tsx` on `/dashboard/driver` all already built. Migration confirmed **not yet applied** to the baseline.
- [~] **More analytics** — the aggregation _pattern_ is real and already covers six views (`getEnrollmentTrend`, `getFeeCollectionTrend`, `getAverageGradesBySubject`, `getAttendanceTrend`, `getTeacherWorkload`, `getLibraryOverdueTrend` in `lib/analytics.ts`). But the two specific views namechecked when this was proposed — a **defaulter trend** and **teacher punctuality** — don't exist as functions yet. Correct that adding them is "new views in the same style," but that work itself hasn't been done.
- [ ] **Lesson Plan approval (HOD workflow)** — not started. `topic_notes.status` exists but is `draft`/`published`/`archived` (`NoteStatus`), not a moderation gate — there's no `pending`/`approved` state and no HOD-scoped update policy on `topic_notes` mirroring `grades_update_hod`/`is_hod_of_subject()`. The reasoning that this reuses an existing shape still holds; it just hasn't been built.
- [ ] **Academic year rollover wizard** — not started. No rollover/wizard code found anywhere in `app`, `lib`, or `components`.
- [ ] **Installment plans (fees)** — not started. No `installment`/`payment_schedule` code anywhere. `invoices`/`fee_structures` still only support the single-total-with-partial-payment model described when this was proposed.
- [ ] **Scholarship/discount codes UI** — backend only. `applyDiscount(invoiceId, discountKobo)` already exists in `lib/actions/fees.ts` (confirmed working, used by the library-fine waiver flow), but **nothing in the UI calls it** for a regular invoice — no discount form/button anywhere in `app` or `components`. This is genuinely just UI work, but it hasn't been done yet.
- [ ] **Bursary dashboard** — not started. `is_bursar()` and bursar-scoped RLS already exist across `invoices`/`payments`/`fee_structures`/`audit_log`, but there's no dedicated bursar-facing page composing them — bursars currently work through the regular admin fees pages.
- [ ] **Bulk email announcements** — not started. No email-sending code found; `announcements` still only delivers in-app.
- [ ] **Lesson notes version diff** — not started. `topic_notes.version` increments correctly (confirmed in the curriculum notes versioning work), but no diff-view component exists to compare two versions.
