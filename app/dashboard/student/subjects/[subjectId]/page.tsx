...
export default async function SubjectTopicsPage({
  ...
  return (
    <div>
      ...
      <h1 className="mb-6 font-display text-2xl font-semibold text-ink">{subject?.name}</h1>

      {Object.entries(topicsByTerm).map(([term, termTopics]) => (
        <div key={term} className="mb-8">
          <h2 className="mb-3 font-display text-lg font-semibold text-ink">Term {term}</h2>
      ...
      {!topics?.length && <EmptyState message="No topics published yet." />}
    </div>
  );
}
